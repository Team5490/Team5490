...
public void drive()  {  //drive method
    ...
    if (driveSpeed < 0.08 && driveSpeed > 0)  {    //sets anything within ± 0.08 to zero...
        this.driveSpeed = 0;                             //...which is used to make turning in place easier
    }
    if (driveSpeed > -0.08 && driveSpeed < 0) {
        this.driveSpeed = 0;
    }
    ...
    if (xSteer > 1) {                               //steering functions
        this.leftSpeed = leftSpeed + Math.pow(xSteer, 3);
        this.rightSpeed = rightSpeed - Math.pow(xSteer, 3);
    }
    if (xSteer < 1) {
        this.leftSpeed = leftSpeed - Math.pow(xSteer, 3);
        this.rightSpeed = rightSpeed + Math.pow(xSteer, 3);
